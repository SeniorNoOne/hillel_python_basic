import csv
import os.path
from src.bookticket.config import files_dir_cl, csv_headers
from src.bookticket.classes.custom_exeptions import CSVHandlerError


class CSVHandler:
    def __call__(self, file_dir: str, headers: dict, mode: str = "r"):
        return self._get_creator(file_dir, headers, mode)

    @staticmethod
    def _get_creator(file_dir: str, headers: dict, mode: str):
        match mode:
            case "r":
...
class CSVReader:
    def __init__(self, file_dir: str, headers: dict):
...
if __name__ == "__main__":
    handler = CSVHandler()
    csv_file = handler(files_dir_cl["tickets"], csv_headers["tickets"], "wr")
    print(handler)
    print(csv_file)
